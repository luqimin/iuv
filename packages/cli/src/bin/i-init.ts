...
program
    ...
    .action(
        async (project: string, options): Promise<void> => {
            // 设置环境变量
            ...
            if (!InitClass) {
                ...
                await install(path.resolve(__dirname, '../../'), INIT_MODULE_ID, program.registry);
                InitClass = safelyRequire(INIT_MODULE_ID);
            }

            new InitClass({ project: projectName, registry: program.registry }).start();
        }
    );
